...
"""Behavior-to-test mapping validator for llvm-libc.

This script validates a small, source-level mapping model:

- behavior IDs are authored in YAML files under `libc/behavior/`
- tests can claim intended coverage with `// @verifies <BEHAVIOR_ID>`
  immediately above a `TEST(...)`-style macro

The validator:
- fails if a behavior ID has no mapped test
- fails if an annotation references an unknown behavior ID
- fails if a behavior ID is declared more than once
- prints a short summary and a Markdown matrix of behavior ID to test name
"""
...
def _extract_behavior_ids_from_yaml(text: str) -> tuple[set[str], set[str]]:
    """Extract tracked IDs from `behaviors:` or legacy `requirements:` lists."""
...
def _discover_behavior_ids(
    ...
    """Find behavior IDs under `behavior_dir` and report any duplicates."""
...
def _render_matrix(
    behavior_ids: list[str], behavior_to_tests: dict[str, set[str]]
) -> str:
    lines = ["| Behavior | Verifying tests |", "|---|---|"]
    for behavior_id in behavior_ids:
        tests = ", ".join(sorted(behavior_to_tests.get(behavior_id, set())))
        lines.append(f"| `{behavior_id}` | {tests} |")
    return "\n".join(lines)
...
def main(argv: list[str]) -> int:
    ...
    print()
    print(_render_matrix(sorted(behavior_ids), behavior_to_tests))
